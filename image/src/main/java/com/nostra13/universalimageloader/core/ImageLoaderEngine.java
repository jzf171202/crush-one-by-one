...
class ImageLoaderEngine {
	...
	private final java.util.Map<Integer, String> cacheKeysForImageAwares = java.util.Collections
			.synchronizedMap(new java.util.HashMap<Integer, String>());
	private final java.util.Map<String, java.util.concurrent.locks.ReentrantLock> uriLocks = new java.util.WeakHashMap<String, java.util.concurrent.locks.ReentrantLock>();

	private final java.util.concurrent.atomic.AtomicBoolean paused = new java.util.concurrent.atomic.AtomicBoolean(false);
	private final java.util.concurrent.atomic.AtomicBoolean networkDenied = new java.util.concurrent.atomic.AtomicBoolean(false);
	private final java.util.concurrent.atomic.AtomicBoolean slowNetwork = new java.util.concurrent.atomic.AtomicBoolean(false);
	...
	void submit(final LoadAndDisplayImageTask task) {
		taskDistributor.execute(new Runnable() {
			@Override
			public void run() {
				java.io.File image = configuration.diskCache.get(task.getLoadingUri());
	...
	private void initExecutorsIfNeed() {
		if (!configuration.customExecutor && ((java.util.concurrent.ExecutorService) taskExecutor).isShutdown()) {
			taskExecutor = createTaskExecutor();
		}
		if (!configuration.customExecutorForCachedImages && ((java.util.concurrent.ExecutorService) taskExecutorForCachedImages)
				.isShutdown()) {
	 ...
	 */
	String getLoadingUriForView(com.nostra13.universalimageloader.core.imageaware.ImageAware imageAware) {
	 ...
	 * exact moment.
	 */
	void prepareDisplayTaskFor(com.nostra13.universalimageloader.core.imageaware.ImageAware imageAware, String memoryCacheKey) {
	 ...
	 *                   will be cancelled
	 */
	void cancelDisplayTaskFor(com.nostra13.universalimageloader.core.imageaware.ImageAware imageAware) {
	...
	void stop() {
		if (!configuration.customExecutor) {
			((java.util.concurrent.ExecutorService) taskExecutor).shutdownNow();
		}
		if (!configuration.customExecutorForCachedImages) {
			((java.util.concurrent.ExecutorService) taskExecutorForCachedImages).shutdownNow();
		}
	...
	java.util.concurrent.locks.ReentrantLock getLockForUri(String uri) {
		java.util.concurrent.locks.ReentrantLock lock = uriLocks.get(uri);
		if (lock == null) {
