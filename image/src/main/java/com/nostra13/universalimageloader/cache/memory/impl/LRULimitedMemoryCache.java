...
public class LRULimitedMemoryCache extends LimitedMemoryCache {
	...
	private final java.util.Map<String, android.graphics.Bitmap> lruCache = java.util.Collections.synchronizedMap(new java.util.LinkedHashMap<String, android.graphics.Bitmap>(INITIAL_CAPACITY, LOAD_FACTOR, true));
	...
	protected android.graphics.Bitmap removeNext() {
		android.graphics.Bitmap mostLongUsedValue = null;
		synchronized (lruCache) {
			java.util.Iterator<java.util.Map.Entry<String, android.graphics.Bitmap>> it = lruCache.entrySet().iterator();
			if (it.hasNext()) {
				java.util.Map.Entry<String, android.graphics.Bitmap> entry = it.next();
